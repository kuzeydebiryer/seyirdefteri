...
async function sorulariUret() {
  ...
  const detaylar = (await Promise.all(adaylar.map((f) => filmDetayGetir(f.disId)))).filter((d) => d?.tagline?.trim())
  ...
  const sorular = rastgeleSec(detaylar, Math.min(SORU_SAYISI, detaylar.length)).map((dogru) => {
    ...
    return { altYazi: `"${dogru.tagline}"`, secenekler }
  })
  return sorular
}

export default function SloganTahmin() {
  ...
  return (
    <div className="max-w-lg">
      ...
      <h1 className="mt-1 mb-1 font-baslik text-2xl text-murekkep">Bu Slogan Hangi Filme Ait?</h1>
      <p className="mb-4 text-sm text-kraft">Afişteki o vurucu cümleyi hangi film taşıyordu?</p>
